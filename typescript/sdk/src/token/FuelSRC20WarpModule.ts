...
export class FuelSRC20WarpModule {
    ...
    await (
      await warpRoute.functions
        .initialize(
          { Address: { bits: owner } },
          mailbox,
          WarpRouteTokenModeInput.NATIVE,
          hook,
          ism,
        )
        .call()
  ...
  public static async update(
    ...
    transactions.push(
      ...
      ...(await this.updateRemoteRoutersAndDecimals(
        warpRoute,
        currentConfig,
        expectedConfig,
      )),
  ...
  static async updateRemoteRoutersAndDecimals(
    warpRoute: WarpRoute,
    currentConfig: HypTokenRouterConfig,
    expectedConfig: HypTokenRouterConfig,
  ): Promise<TransactionResponse[]> {
    ...
    updateTransactions.push(
      ...
      ...(await this.setRemoteRoutersDecimals(
        warpRoute,
        routesToEnroll,
        decimalsToSet,
      )),
  ...
  static async setRemoteRoutersDecimals(
    ...
    for (const domain of domains) {
      const { transactionResponse } = await (
        await warpRoute.functions
          .set_remote_router_decimals(domain, decimals)
          .call()
      ).waitForResult();
