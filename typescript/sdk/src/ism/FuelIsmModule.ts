...
export class FuelIsmModule {
    ...
    const { waitForResult: wait } =
      routingConfig.type === IsmType.ROUTING
        ? await (contract as DomainRoutingIsm).functions
            .initialize_with_domains(owner, domains, modules)
            .call()
        : await (contract as DefaultFallbackDomainRoutingIsm).functions
            .initialize_with_domains(owner, mailboxId, domains, modules)
            .call();
    await wait();
  ...
  static async deployAggregationIsm(
    ...
    const { contract } = await waitForResult();
    await (await contract.functions.initialize(owner).call()).waitForResult();
    await (
      await contract.functions.set_threshold(threshold).call()
    ).waitForResult();
    for (const module of modules) {
      await (
        await contract.functions.enroll_module({ bits: module }).call()
      ).waitForResult();
    }
  ...
  static async deployMultisigIsm(ismConfig: IsmConfig, signer: WalletUnlocked) {
    const { type, threshold, validators } =
    ...
    for (const validator of validators) {
      const bits = `0x000000000000000000000000${validator.slice(
        2,
      )}` as B256AddressEvm;
      await (
        await contract.functions.enroll_validator({ bits }).call()
      ).waitForResult();
    }
